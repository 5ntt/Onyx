...
LOGO_PATH = BASE_DIR / "assets" / "onyx_logo.jpg"
...
BRAND = "✦ أونيكس بوت | ONYX BOT"
WELCOME = (
    f"<b>{BRAND}</b>\n\n"
    "بوت تنزيل وسائط بواجهة احترافية. أرسل رابطًا عامًا من منصة مدعومة، "
    "ثم اختر الجودة أو صيغة الصوت.\n\n"
    "<b>فيديو:</b> 360p • 480p • 720p • 1080p • أفضل جودة\n"
    "<b>صوت:</b> MP3 128 • 192 • 320 kbps\n\n"
    "<i>استخدمه فقط مع المحتوى الذي تملكه أو المسموح لك بتنزيله. "
    "لا يتجاوز DRM أو المحتوى الخاص/المقفل.</i>"
)
...
def main_menu() -> InlineKeyboardMarkup:
    rows = [
        ...
        [InlineKeyboardButton("✨ عن أونيكس", callback_data="about")],
    ]
    if SUPPORT_URL:
    ...
    if LOGO_PATH.exists():
        with LOGO_PATH.open("rb") as f:
            await update.message.reply_photo(
                photo=InputFile(f, filename="onyx_logo.jpg"),
                caption=WELCOME,
                parse_mode=ParseMode.HTML,
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "home":
        await safe_edit(query, WELCOME, main_menu())
        return
    if data == "new_link":
        context.user_data.pop("url", None)
        context.user_data.pop("media_info", None)
    ...
    if data == "about":
        await safe_edit(
            query,
            "<b>✨ ONYX BOT</b>\n\n"
            "واجهة عربية سريعة لتنزيل الوسائط العامة بصيغ وجودات مختلفة، "
            "مع إدارة ضغط الطلبات وإحصائيات محلية وتنظيف تلقائي للملفات.\n\n"
            "<code>build(); create(); improve(); repeat();</code>",
            back_menu(),
        )
        return
